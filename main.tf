...
locals {
  ...
  metric_name = join("", [for element in split("-", lower(replace(var.metric_name, ".", "-"))) : title(element)])
  alarm_name = var.alarm_name == null ? "${local.domain_name_snake}ElasticSearch${var.statistic}${local.metric_name}${var.comparison}${var.threshold}In${var.evaluation_periods}PeriodsOf${var.period}" : var.alarm_name
}

# Retrieve the requested ElasticSearch domain
